        ...
        {isUploadingOrProcessing && (
          <p className="w-full text-center text-gray-300 mb-2">
            {currentStatus === 'uploading'
              ? t('products.uploading_percent', { percent: Math.round(uploadProgress) })
              : t('products.processing_percent', { percent: Math.round(uploadProgress) })}
          </p>
        )}
        {currentStatus !== 'success' && (
          <>
              ...
              >
                {isUploadingOrProcessing ? (
                  ...
                  <>
                    ...
                    {t('products.use_saved_body_image')}
                  </>
                )}
              </button>
            )}
            ...
            >
              ...
              {uploaderOpen ? t('products.close_uploader') : t('products.upload_new_photo')}
            </button>
          </>
        )}
      </div>
          ...
          <div className="flex items-center justify-center w-full mb-4">
            <label htmlFor="image-upload" className="flex flex-col items-center justify-center w-full h-64 bg-gray-900 border border-dashed border-gray-700 rounded-md cursor-pointer hover:bg-gray-800">
              <div className="flex flex-col items-center justify-center text-body pt-5 pb-6">
                ...
                <p className="mb-2 text-sm text-gray-400"><span className="font-semibold">{t('products.click_to_upload')}</span> {t('products.or_drag_drop')}</p>
          ...
          <div className="flex gap-3 justify-center">
            ...
            >
              {currentStatus === 'uploading' ? (
                <>
                  <CircularProgressIndicator progress={uploadProgress} />
                  <span>{t('products.uploading_percent', { percent: Math.round(uploadProgress) })}</span>
                </>
              ) : currentStatus === 'processing' ? (
                <>
                  <CircularProgressIndicator progress={uploadProgress} />
                  <span>{t('products.processing_percent', { percent: Math.round(uploadProgress) })}</span>
                </>
              ) : (
                t('products.submit_for_try_on')
              )}
            </button>
          ...
          {currentStatus === 'error' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="mt-3 text-sm text-red-400 flex items-center gap-2 justify-center">
              ...
              <button onClick={handleRetry} className="ml-2 text-blue-400 hover:text-blue-300 flex items-center gap-1">
                <RotateCcw className="w-4 h-4" /> {t('products.try_again')}
              </button>
            </motion.div>
          )}

          {currentStatus === 'idle' && errorMessage && <p className="mt-3 text-sm text-red-400 text-center">Error: {errorMessage}</p>}
          {currentStatus === 'idle' && !errorMessage && <p className="mt-3 text-sm text-gray-400 text-center">{t('products.status_ready')}</p>}
        </div>
      )}
      ...
      {currentStatus === 'success' && processedImageUrl && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }} className="mt-8 bg-gray-900 p-6 rounded-xl border border-white/6 text-center">
          <h3 className="font-semibold text-xl text-white mb-4 flex items-center justify-center gap-2">
            <CheckCircle className="w-6 h-6 text-green-500" /> {t('products.try_on_ready')}
          </h3>
          {responseMetadata && (
          ...
          <div className="flex justify-center gap-4">
            <a href={processedImageUrl} download="diva-haus-tryon.png" className="bg-green-600 text-white font-semibold px-6 py-3 rounded-md shadow-md hover:bg-green-700 transition">{t('products.download')}</a>
            <button className="bg-blue-600 text-white font-semibold px-6 py-3 rounded-md shadow-md hover:bg-blue-700 transition">{t('products.share')}</button>
